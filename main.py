...
def test_prediction(src_dir='data', dataset_id=0, split_name='train', log_dir='logs', is_online_fig=False):
...
def test_update(src_dir='data', dataset_id=0, split_name='train', log_dir='logs', map_resolution=None, is_online_fig=False):
...
def particle_SLAM(src_dir, dataset_id=0, split_name='train', running_mode='test_SLAM', log_dir='logs'):
    ...
    mov_cov = np.array([[5e-4, 0, 0],[0, 5e-4, 0],[0, 0 , 5e-4]])
    ...
    if running_mode == 'test_prediction':
        tes_prediction(src_dir, dataset_id, split_name, log_dir)
        exit(1)
